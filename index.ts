import { Journal, parse } from "pta-tools";
...
import { AmountType, JournalEntries, Posting } from "pta-tools/build/types";
...
async function createAccounts(names, dictionary, client) {
  let errors = await client.createAccounts(
    names.map((name, i) => {
      return {
        ...
        ledger: 1,
        code: 718,
        user_data_128: accountStringToUserData(name, dictionary),
        reserved: 0,
        flags: 0,
        debits_pending: 0n,
        debits_posted: 0n,
  ...
  let readBack = await client.lookupAccounts(
    names.map((name) => accountStringToUserData(name, dictionary))
  );
  names.forEach((name) => {
    ...
    if (!found) {
      throw new Error(
        `No account with name ${name} and id ${accountId} was created!`
      );
    }
  });
}
...
async function createTransfers(journal: Journal, dictionary, client: Client) {
  let transferData: ITransferData[] = [];
  let id = 1n;
  journal.forEach((obj: JournalEntries) => {
    if ("entries" in obj && Array.isArray(obj.entries)) {
      ...
      for (let i = 0; i < obj.entries.length; i++) {
        ...
        if ("account" in entry && typeof entry.amount === "string") {
          console.log("adding " + entry.amount, typeof entry.amount);
        ...
        if (i !== equalizer) {
          ...
          if ("account" in entry) {
            transferData.push({
              id,
              from: entry.account,
              to: (obj.entries[equalizer] as Posting).account,
              amount: entry.amount,
            });
          }
          id++;
  ...
  let errors = await client.createTransfers(
    transferData.map((data) => makeTransfer(data, dictionary))
  );
  console.log(
    "transfers created",
    errors,
    errors.map((obj) => {
      return `${obj.index}: ${obj.index} - ${CreateTransferError[obj.index]}`;
    })
  );
  const range = [...Array(transferData.length).keys()].map((x) =>
    BigInt(x + 1)
  );
  ...
  for (let i = 0; i < transferData.length; i++) {
    ...
    if (
      transferData[i].from ==
      accountUserDataToString(readBack[i].debit_account_id, dictionary)
    ) {
      console.log(`FROM OK: ${transferData[i].from}`);
    } else {
      console.error(
        `FROM NOT OK: ${transferData[i].from} != ` +
          accountUserDataToString(readBack[i].debit_account_id, dictionary)
      );
    }
    if (
      transferData[i].to ==
      accountUserDataToString(readBack[i].credit_account_id, dictionary)
    ) {
      console.log(`TO OK: ${transferData[i].to}`);
    } else {
      console.error(
        `TO NOT OK: ${transferData[i].to} != ` +
          accountUserDataToString(readBack[i].credit_account_id, dictionary)
      );
    }
    if (transferData[i].amount == readBack[i].amount.toString()) {
      ...
      console.error(
        `AMOUNT NOT OK: ${transferData[i].amount} != ` +
          readBack[i].amount.toString()
      );
    }
  }
}
